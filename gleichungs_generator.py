...
def beautify_equation(lhs: sp.Expr, rhs: sp.Expr, visual_complexity: str = "mixed") -> str:
...
def build_L3(rng: random.Random, sol: Fraction, cfg: Dict, pattern_index: int = None) -> Equation:
    ...
    if pattern == "plus":
        ...
        if _fraction_ok(Fraction(q)):
            ...
            if cfg.get("visual_complexity", "mixed") == "mixed":
                ...
                text = beautify_equation(lhs, rhs, cfg.get("visual_complexity", "clean"))
...
def build_L5(rng: random.Random, sol: Fraction, cfg: Dict) -> Equation:
    ...
    if variant == "simple":
        ...
        if _fraction_ok(c):
            ...
            rhs = sp.Rational(c.numerator, c.denominator) if c.denominator != 1 else c.numerator
            text = beautify_equation(lhs, rhs, cfg.get("visual_complexity", "mixed"))
        ...
        while attempts < 30:
            ...
            if _fraction_ok(g) and math.lcm(c, f) <= MAX_ABS:
                lhs = (a * x + b) / c + (d * x + e) / f
                rhs = sp.Rational(g.numerator, g.denominator) if g.denominator != 1 else g.numerator
                text = beautify_equation(lhs, rhs, cfg.get("visual_complexity", "mixed"))
                excluded = set()
    ...
    rhs = sp.Rational(sol.numerator, 2 * sol.denominator) if sol.denominator != 1 else sp.Rational(sol.numerator, 2)
    text = beautify_equation(lhs, rhs, cfg.get("visual_complexity", "mixed"))
    return Equation("L5", sp.Eq(lhs, rhs, evaluate=False), text, sol)
...
def validate(eq: Equation) -> bool:
    ...
    try:
        ...
        if abs(sol_frac - eq.solution) > Fraction(1, 1000):  # Toleranz für Rundungsfehler
            return False
...
def solve_steps(eq: Equation, cfg: Dict) -> List[SolveStep]:
    ...
    if eq.excluded:
        ...
        steps.append(SolveStep(f"Definitionsmenge: {excl_str}", eq.sympy_eq.lhs, eq.sympy_eq.rhs))
    ...
    if expr.has(x):
        ...
        if len(coeffs) >= 2:
            ...
            if g > 1:
                A //= g
                B //= g
                steps.append(SolveStep(f"Durch gemeinsamen Teiler {g} kürzen", 
                                      A * x + B, sp.Integer(0)))
            ...
            if A != 0:
                ...
                if cfg.get("solutions", {}).get("improper_and_mixed", True):
                    if sol.q != 1:  # Ist ein Bruch
                        ...
                        if mixed != str(sol):
                            steps.append(SolveStep(f"Als gemischte Zahl: {mixed}", x, sol))
        ...
        for problem in by_level[level]:
            ...
            for step in problem.steps:
                if step.description_de.startswith("Definitionsmenge"):
                    ...
                    eq_text = beautify_equation(step.lhs, step.rhs, cfg.get("visual_complexity", "mixed"))
