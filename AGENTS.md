...
* [x] Nur **eine Variable x** pro Aufgabe; jede Gleichung **eindeutig lösbar** (keine Division durch 0, keine Widersprüche, keine Identitäten ohne eindeutige Lösung).
* [x] **5 Schwierigkeitslevel**:

  1. [x] **Einfach**: x nur auf **einer Seite** (z. B. `3x-7=12`, `5-2x=17`, `23=7+2x`).
  2. [x] **Beide Seiten**: `ax+b = dx+e`.
  3. [x] **Eine Klammer**: je 2× **+**, **–**, **×** vor der Klammer (× auch implizit).
  4. [x] **Gemischte Klammern**: Kombinationen aus +/–/× und mehreren Klammern.
  5. [x] **Bruchgleichungen**: von einfach (`(1/2)x=5`) bis schwer (verschachtelte Produkte/Quotienten).
* [x] **Lösungen bevorzugt als Brüche** (kein Dezimalzwang); Rechenweg im Lösungsblatt **schrittweise** nachvollziehbar.
* [x] **Word-Export**: Zwei Dateien `.docx`: Arbeitsblatt (Aufgaben, mit Leerraum), Lösungsblatt (nummerierte Lösungen + Rechenweg).
* [x] **Sprache**: Deutsch (Aufgaben & Rechenwege).

### Should-have

* [x] **Verteilung (Standard)**: L1=4, L2=4, L3=6, L4=4, L5=5 ⇒ **gesamt 23 Aufgaben**.
* [x] **Parametrisierbare Anzahl** pro Level; globaler **Zufallssamen** (reprozierbare Sätze).
* [x] **Bruchdarstellung** optional als **unechter Bruch** oder **gemischte Zahl**.
* [x] **Genügend Abstand** zwischen Aufgaben (z. B. 4–6 leere Zeilen oder Absatzabstände in `docx`).
...
* [x] Keine Nichtlinearitäten, keine Potenzen/Quadrate, keine Systeme.
* [x] Kein PDF-Export (nur `.docx`).
* [x] Keine grafischen Lösungshilfen.
...
* [x] **Config**: Parameter (Anzahl pro Level, Zahlenbereiche, Ziel‑Bruchhäufigkeit, `visual_complexity`, Zufallssamen).
* [x] **Sampler**: zieht eine Ziel‑Lösung r = p/q als Rationalzahl (bevorzugt Nicht‑Ganzzahl), um gezielt Aufgaben zu konstruieren, die auf x = r führen.
* [x] **Builder L1…L5**: Vorlagen/Algorithmen je Level, die Terme so zusammensetzen, dass nach Vereinfachung eine lineare Gleichung entsteht und x = r die Lösung ist.
* [x] **Validator**: prüft Eindeutigkeit (kein Koeffizient 0, keine Identität), Definitionsmenge (Nenner ≠ 0), kein Verbot schwacher Formen.
* [x] **Beautifier**: setzt Formatpräferenzen (implizites Mal „2x“, Klammerstile, Minus vor Klammer). Gesteuert durch `visual_complexity` = "clean"|"mixed".
* [x] **Solver + StepTracer**: löst mit SymPy und erzeugt Schritt‑für‑Schritt‑Rechenwege (Klammern auflösen → auf eine Seite bringen → zusammenfassen → durch Koeffizienten teilen; bei Brüchen zusätzlich Hauptnenner bilden, Ausschlusswerte nennen).
* [x] **Deduper**: erkennt Duplikate per kanonischer Form (normalisierte Gleichung & Lösung) und verwirft Wiederholungen.
* [x] **Renderer (DOCX)**: erstellt `arbeitsblatt.docx` (nur Aufgaben, mit Abstand) & `loesungsblatt.docx` (nummerierte Lösungen + Rechenweg).
...
* [x] `Equation`: `{ id, level, sympy_eq: Eq, text: str, solution: Rational, excluded: set, template: str, params: dict }`
...
1. [x] Ziehe zufällig p ∈ \[−12, 12] \ {0}, q ∈ \[2, 12].
2. [x] Setze r = Rational(p, q). Mit Wahrscheinlichkeit α (z. B. 0.8) erzwinge Nicht‑Ganzzahl (falls q p teilt, neu ziehen).

### Builder‑Strategien (je Level)

**Level 1 (x auf einer Seite)**

* [x] Muster: `ax + b = c` oder `a − bx = c` oder `c = d + ex`.
* [x] Vorgehen: Ziehe a ≠ 0, b, c zufällig; erzwinge c = a·r + b (bzw. analog), damit Lösung r ist.
* [x] Variation: Zufällig linke/rechte Seite vertauschen; Vorzeichen von a, b mischen.

**Level 2 (x auf beiden Seiten)**

* [x] Muster: `ax + b = dx + e`.
* [x] Ziehe a ≠ d für eindeutige Lösung.
* [x] Setze z. B. b = d·r + e − a·r.
* [x] Parameter a, d, e zufällig, dann b berechnen.
* [ ] Prüfe |a − d| nicht zu klein (zur Schrittvielfalt; optional Normierung auf positiven x‑Koeffizienten).

**Level 3 (eine Klammer)**

* [x] Muster (+): `k + (mx + n) = px + q`.
* [x] Muster (−): `k − (mx + n) = px + q`.
* [x] Muster (× implizit): `t(mx + n) = px + q`.
* [x] Vorgehen: Wähle k, m, n, t, p, q zufällig und erzwinge Gleichung so, dass x = r erfüllt ist (z. B. p·r + q = k + (m·r + n)).
* [ ] `visual_complexity="mixed"`: häufiger implizites Mal (`13(2x−7)`).

**Level 4 (gemischte Klammern)**

* [x] Muster: `a(mx+n) + b − (cx+d) = p − (qx+r) + s(tx+u)`.
* [x] Vorgehen: Werte beidseitig bei x = r gleichsetzen und dann eine Konstantenvariable (z. B. u) auflösen, damit die Bedingung exakt stimmt.
* [x] Sicherstellen, dass nach Expandieren linear bleibt und x‑Koeffizient ≠ 0.

**Level 5 (Bruchgleichungen)**

* [x] Einfach: `(a/b)·x = c` ⇒ r = (b·c)/a oder umgekehrt c = (a/b)·r.
* [x] Mittel: `(ux+v)/w = y` oder Summe mehrerer Brüche mit Hauptnenner.
* [x] Anspruchsvoll (lineare Bruchterme beidseitig):
  Konstruiere
...
* [x] Eindeutigkeit: Nach Expand/Collect muss Gleichung A·x + B = 0 mit A ≠ 0 ergeben.
* [x] Domäne: Für alle Nennerterme Liste der Ausschlusswerte (z. B. 2x − 3 ≠ 0 ⇒ x ≠ 3/2).
* [ ] Bruchquote: Erzwinge Anteil β (z. B. 70–90 %) nicht‑ganzzahliger r; notfalls neu sampeln.
* [x] Kanonische Form: `sympy.together() → expand() → collect(x) → Eq(lhs, rhs)`; String‑Hash in Set speichern.

### StepTracer (Rechenweg‑Heuristik)

1. [x] Klammern auflösen (`expand`).
2. [x] Brüche beseitigen: mit Hauptnenner multiplizieren; Ausschlusswerte explizit notieren.
3. [x] x‑Terme auf eine Seite, Konstanten auf die andere.
4. [x] Zusammenfassen (`collect(x)`), ggf. mit −1 multiplizieren, um führenden Koeffizienten positiv zu machen.
5. [x] Durch Koeffizienten teilen und als Rational formatiert ausgeben; optional zusätzlich gemischte Zahl.

### DOCX‑Ausgabe

* [x] **Arbeitsblatt**: Level‑Überschriften, dann nummerierte Aufgaben. Nach jeder Aufgabe Absatzabstand (z. B. 24–36 pt × 4–6 Zeilen) + optional Leerzeilen.
* [x] **Lösungsblatt**: gleiche Nummerierung; unter jeder Aufgabe Schrittfolge als Liste.
...
* [x] **Python** ≥ 3.10
* [x] **SymPy** (Algebra, Bruchformate)
* [ ] **Spire.Doc for Python (Free Edition)** für hochwertige **Word‑Gleichungen** via **MathML/OMML** – *keine lokale LaTeX‑Installation nötig*.
* [x] Fallback (**ohne** Spire): `python-docx` mit **Unicode‑Mathe** (gut lesbar, aber nicht so hochwertig wie Word‑Equation).
...
* [x] **GN‑3 Linearität & Eindeutigkeit:** Jede Gleichung ist nach Expand/Together/Collect **vom Grad 1** mit **genau einer Lösung**. **Ausschlusswerte** (lineare Nenner = 0) **dokumentieren** und **≠ Lösung**.
...
* [x] **GN‑5 Ziel‑Lösung:** Ziehe gezielt **Nicht‑Ganzzahl‑Lösungen** (Rational), bevorzugt „unschöne“ Brüche; achte, dass Zähler/Nenner **≤ 120** bleiben.
...
* [x] Zeige **zwei Formate** der Lösung: **unechter Bruch** **und** **gemischte Zahl**. Keine Dezimaldarstellung. Notiere **Definitionsmengen** klar.
